...
oldwd <- getwd()
setwd("/home/doma/0dssc/smds/statMethodsProject/input_amazon/")
...
apply(data[1:n,], 1, 
      function(x) write.table(data.frame(x[1]), 
                              file = gsub(" ","", paste("../input_amazon/",x[4],sep="")), row.names = FALSE, col.names = FALSE))
...
write.csv(data1, file = "../input_amazon/control.txt", row.names = FALSE)       
